...
def plot_acc(acc_list, dataset_name, name, imgs_path):
    ...
    # 设置图表的标题及其字体大小
    plt.title(f'Accuracy_{dataset_name}[{name}]', fontsize=16)
    ...
    # 在图表上标注最大准确率及其对应的轮数
    plt.text(epochs, max_acc, f'Max Acc: {max_acc * 100:.2f}% at Epoch {max_epoch}', ha='right', va='bottom',
             fontsize=10)
    # 在图表上标注最后一轮的准确率
    plt.text(1, 0, f'Last Acc: {last_acc * 100:.2f}%', ha='right', va='bottom', fontsize=10,
    ...
    # TODO 文件名
    filename = f'{imgs_path}/{dataset_name}/{dataset_name}_ep{epochs}_{current_time}_Accuracy.png'
